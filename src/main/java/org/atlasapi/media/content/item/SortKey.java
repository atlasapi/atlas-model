...
public enum SortKey {
    ...
    BROADCAST("75") {
        ...
        protected String generateFrom(Item item) {
            ...
            for ( Version version : item.versions()) {
                for (Broadcast broadcast : version.broadcasts()) {
                    if(firstBroadcast == null || broadcast.transmissionTime().isBefore(firstBroadcast)) {
                        firstBroadcast = broadcast.transmissionTime();
                    }
                }
            }
